...
public class PdSysWebSecurityConfigurerAdapter extends WebSecurityConfigurerAdapter  {
	...
	@Autowired
	private PdSysAuthSuccessHandler authSuccessHandler;
	@Autowired
	private PdSysAuthFailureHandler authFailureHandler;
...
//	                	.defaultSuccessUrl("/index")// 设定这个有时候会出现status=999的错误，我也不知道为毛，先用下面的handler跳转一下
                ...
                .and()
                	.logout()
	                	...
	                	.permitAll()//注销行为任意访问 
        		.and()
        			.csrf().disable();//ajax提交会被阻挡 403 见https://segmentfault.com/q/1010000012743613
//                .and()
